...
static void merge(item *a, item *b, unsigned int inzio, unsigned int centro, unsigned int fine, bool (*minorEqual)(item a, item b, item args), item args) {
    unsigned int i, j;
    ...
    for (k = inzio; i <= centro && j <= fine; k++) { // Confronto gli array sinistro e destro
        if ((*minorEqual)(a[i], a[j], args))               // Se l'elemento nel sinistro è minore o uguale (oppure quello destro maggiore o uguale) all'altro copio di conseguenza
            b[k] = a[i++];
        else
            b[k] = a[j++];
    }
...
/* Componente ricorsiva del Merge Sort
   minorEqual(item a, item b) = restituisce true se a è minore o ugule a b secondo il criterio di ordinamento */
static void ordinaRicorsivo(item *a, item *b, unsigned int inizio, unsigned int fine, bool (*minorEqual)(item a, item b, item args), item args) {
    int centro = (inizio + fine) / 2; // Calcolo l'indice di metà vettore
    if (inizio >= fine) {             // Se ho un vettore unitario mi interrompo
        return;
    }

    ordinaRicorsivo(a, b, inizio, centro, minorEqual, args);   // Ordina a sinistra
    ordinaRicorsivo(a, b, centro + 1, fine, minorEqual, args); // Ordina a destra
    merge(a, b, inizio, centro, fine, minorEqual, args);       // Unisco i due array
}
...
void mergeSort(array a, bool (*minorEqual)(item a, item b, item args), item args) {
    ...
    ordinaRicorsivo(a->Objects, b->Objects, inizio, fine, minorEqual, args);
}