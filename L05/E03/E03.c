...
size_t numeroCorse(FILE *stream) { // Restituisce il numero di corse dentro il file
    size_t righe;
    fscanf(stream, "%zu", &righe);
    return righe;
}

size_t parseCorse(char *filename) {                                    // Funzione che effettua il parse delle corse
    memset(ORDINAMENTI, NULL, ordinamentiPossibili * sizeof(corsa *)); // Rendo null tutti i valori del puntatore
    FILE *stream = fopen(filename, "r");
    checkFilestream(stream);
    size_t nCorse            = numeroCorse(stream);
    ORDINAMENTI[nonOrdinato] = (corsa *)calloc(sizeof(corsa), nCorse); // Alloco la memoria per il primo ordinamento
    ...
    int i         = 0;
    while (scanCheck == 11) {
        corse[i].CodiceTratta = (char *)calloc(sizeof(char), strlen(CodiceTratta)); // Alloco la memoria necessaria
        corse[i].Partenza     = (char *)calloc(sizeof(char), strlen(Partenza));     // Alloco la memoria necessaria
        corse[i].Destinazione = (char *)calloc(sizeof(char), strlen(Destinazione)); // Alloco la memoria necessaria
...
void freeAllMemory(size_t numeroCorse) {                   // Libera la memoria allocata leggendo gli ordinamenti
    for (size_t j = 0; j < numeroCorse; j++) {             // Per ogni corsa
        free(ORDINAMENTI[ORDINE_ATTUALE][j].CodiceTratta); // Libero la memoria
        free(ORDINAMENTI[ORDINE_ATTUALE][j].Partenza);
        free(ORDINAMENTI[ORDINE_ATTUALE][j].Destinazione);
    }

    for (size_t i = 0; i < ordinamentiPossibili; i++) { // Per tutti gli ordinamenti possibili
        free(ORDINAMENTI[i]);                           // Libero il puntatore
    }
}
...
bool confrontaCampi(corsa *elemento1, corsa *elemento2, ordinamento campo) { // Ricevute due corse in ingresso ed un campo di confronto, restituisce se la prima è maggiore o uguale alla seconda
    switch (campo) {                                                         // Per ogni campo ho un processo differente
        ...
        case tratta: {
            return ordineAlfabetico(elemento2->CodiceTratta, elemento1->CodiceTratta);
            break;
        }
...
void ordina(int numeroCorse, ordinamento ordine) { // Funzione che ordina il log in base a come richiesto (è un Merge Sort)
    if (ORDINAMENTI[ordine] != NULL) {             // Se l'ordinamento è già stato effettuato aggiorno il puntatore, effettuo il controllo su un campo della tratta
        ORDINE_ATTUALE = ordine;
        return;
    }

    ORDINAMENTI[ordine] = (corsa *)calloc(sizeof(corsa), numeroCorse);                     // Alloco la memoria necessaria
...
void promptMenu(int numeroCorse) {
    while (true) {
        ...
        switch (lettura) {
            ...
            case 3: {
                freeAllMemory(numeroCorse);
                puts("Scrivi il nome del nuovo file:");
                char filename[MAX_STRING];
                gets(filename);
                gets(filename); // Doppia lettura per evitare errori
                printf("\n");
                numeroCorse = parseCorse(filename);
            }
            default: {
                break;
            }
...
int main() {
    puts("Scrivi il nome del file:");
    char filename[MAX_STRING];
    gets(filename);
    printf("\n");
    size_t numeroCorse = parseCorse(filename);
    promptMenu(numeroCorse);
    return 0;
}