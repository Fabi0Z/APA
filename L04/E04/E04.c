...
typedef enum { data,
               ...
               arrivo,
               ora,
               nonOrdinato } ordinamento; // Enum per indicare il tipo di ordinamento
ordinamento ORDINE_ATTUALE = nonOrdinato;
...
void parseCorse(FILE *stream, corsa *corse) { // Funzione che effettua il parse delle corse
    char CodiceTratta[MAX_STRING];
    char Partenza[MAX_STRING];
    char Destinazione[MAX_STRING];
    uint8_t Giorno, Mese;
...
void printLog(int numeroCorse, corsa corse[numeroCorse]) { // Stampa il contenuto dell'array di corse
    FILE *stream;
...
void promptOrdinamento(int numeroCorse, corsa corse[numeroCorse]) {
    ...
    ordinamento ordine;
    scanf("%d", &ordine);
    ordina(numeroCorse, corse, ordine);
    printf("\n");
}
...
void ricercaLineare(int numeroCorse, corsa corse[numeroCorse], char strPartenza[MAX_STRING], FILE *stream) { // Ricerca le tratte che portano ad una destinazione a partire da un array non ordinato
...
void cercaTratta(int numeroCorse, corsa corse[numeroCorse]) { // Ricerca e stampa le tratte che portano ad una determinata destinazione
    ...
    if (ORDINE_ATTUALE == partenza) {
        ricercaDicotomica(numeroCorse, corse, strPartenza, stream); // Se il vettore è ordinato
    } else {
        ricercaLineare(numeroCorse, corse, strPartenza, stream); // Altrimenti eseguo la ricerca lineare
    }
...
void promptMenu(int numeroCorse, corsa corse[numeroCorse]) {
    while (true) {
        ...
        switch (lettura) {
            ...
            case 0: {
                printf("\n");
                printLog(numeroCorse, corse);
                break;
            }
            case 1: {
                printf("\n");
                promptOrdinamento(numeroCorse, corse);
                break;
            }

            case 2: {
                printf("\n");
                cercaTratta(numeroCorse, corse);
                break;
            }
...
int main() {
    ...
    parseCorse(corseStream, corse);
    promptMenu(nCorse, corse);
    return 0;
}